...
$sheet->WriteString(1, 1, "InvoiceNo", $xml_styles['column_names']);
$sheet->WriteString(1, 2, "QuoteNo", $xml_styles['column_names']);
$sheet->WriteString(1, 3, "Customer", $xml_styles['column_names']);
$sheet->WriteString(1, 4, "InvoiceDate", $xml_styles['column_names']);
$sheet->WriteString(1, 5, "DueDate", $xml_styles['column_names']);
$sheet->WriteString(1, 6, "Memo-End User", $xml_styles['column_names']);
$sheet->WriteString(1, 7, "Reservation Type", $xml_styles['column_names']);
$sheet->WriteString(1, 8, "Description	", $xml_styles['column_names']);
$sheet->WriteString(1, 9, "ItemQuantity", $xml_styles['column_names']);
$sheet->WriteString(1, 10, "ItemRate", $xml_styles['column_names']);
$sheet->WriteString(1, 11, "ItemAmount", $xml_styles['column_names']);
$sheet->WriteString(1, 12, "ServiceDate", $xml_styles['column_names']);
$sheet->WriteString(1, 13, "Class", $xml_styles['column_names']);
...
//    var_dump($invoice['items']);
    ...
    $quoteNumber = $quote->GetQuote()['quote_number'];
//    var_dump($quoteNumber);exit;
...
//        }
        ...
        $dataItem = array(
            'customer' => $agency_info['agency_name'] ,//TBD Vendor
            'memoEndUser' => $memoEndUser,
...
//     Output
    ...
    $deptOffice = $agentOffice .'-'. $agentDept;
    $sheet->WriteNumber($row, 1, $dataItem['invoiceNumber'], $xml_styles['row_cell']);

    $sheet->WriteString($row, 2, $dataItem['quoteNumber'], $xml_styles['row_cell']);
    $sheet->WriteString($row, 3, $dataItem['customer'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 4, $dataItem['transactionDate'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 5,$dataItem['dueDate'], $xml_styles['row_cell']);
    $sheet->WriteString($row, 6, $dataItem['memoEndUser'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 7, $dataItem['reservationType'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 8, $dataItem['lineDescription'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 9, $dataItem['lineQuantity'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 10,"1", $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 11, $dataItem['linePrice'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 12, $dataItem['transactionDate'], $xml_styles['row_cell']);
    $sheet->WriteNumber($row, 13, $deptOffice, $xml_styles['row_cell']);
...
function GetInvoices($search_string, $offset, $limit, $sort_by, $visible_paid_status)
{
    ...
    $query .= "AND inv.creation_date < '2021-01-15'";
